...
?>
        <main class="add-course">
            ...
            <a href="add_item.php?id=<?php echo $class_row['class_id'] ?>" class="btn btn-info pull-left" id="addd">Add Lecture to Class</a> 
            
            <a id="<?php echo $class_row['class_id'] ?>" href="#" class="btn btn-danger pull-left end">End Course</a>
            <?php } ?>
...
<script type="text/javascript">
        $(function() {
            $(".end").click(function() {
                var element = $(this);
                var end_id = element.attr("id");
                var info = 'id=' + end_id;
                if (confirm("Are you sure you want to end this course?")) {
                    $.ajax({
                        type: "POST",
                        url: "end.php",
                        data: info,
                        success: function() {
                            document.location = 'view_class.php?id=<?php echo $class_row["class_id"]?>';
                        }
                    });
                    $(this).parents(".show").animate({
                            backgroundColor: "#003"
